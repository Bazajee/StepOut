...
async function submitForm() {
  ...
  const urlFecth = showPasswordInput.value
    ? "/api/sign_in"
...
<template>
  <div class="flex flex-col w-fit m-auto">
    <h1 v-if="showPasswordInput" class="text-center font-bold inline-block">
      Create Account
    </h1>
    ...
    <span
      v-else-if="!errorLogin && !errorCreateAccount"
      class="pb-4 flex justify-end"
    >
      <span
        class="text-blue-500 hover:bg-gray-200 transition-colors duration-300 cursor-pointer"
        @click="handleClickCreateAcc()"
        >Create Account</span
      >
    </span>
    <span v-if="errorLogin" class="pb-4 flex justify-end">
      <span class="text-red-500 transition-colors duration-300 cursor-pointer"
